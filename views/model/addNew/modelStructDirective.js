...
(function (factory) {
     ...
     *          previewAuth: 传递的预览权限
     */
    app.directive('modelStruct', ['$document', 'modelService', function ($document, modelService) {
        return {
            restrict: 'EA',
            replace: true,
            templateUrl: 'model/addNew/model-struct-directive.htm',
            scope: {
                model: '=',
                silenceScreen: '=',
                preview: '&',
            ...
            link: function ($scope, element, attrs) {
                // ************************************leichen13 @2017-09-19************************************************
                // $scope.model.choose = false; /*默认顺序模式未选中；leichen13 @2017-09-19*/

                // $scope.getIsSequentialModel = function(){ /*获取是否为顺序模型的接口*/
                //   modelService.isSequentialModel()
                //       .then(function(result){
                //           $scope.choose = Number(result.value.isSequentialModel) === 1 ? true : false;
                //   });
                // }
                // $scope.getIsSequentialModel();
