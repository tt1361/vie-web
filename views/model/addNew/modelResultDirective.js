...
(function (factory) {
    ...
    app.directive('modelResult', [
        ...
        'modelConstant',
        'CONSTANT',
        'dimensionService', function ($http, ngDialog, $timeout, $q, $rootScope, modelService, dialogService, modelConstant, CONSTANT, dimensionService) {
            return {
                restrict: 'EA',
                replace: true,
                ...
                link: function ($scope, element, attrs) {
                    ...
                    $scope.submitRemark = function (item) {
                        ...
                        if ($rootScope.isTask == 0) {
                            ...
                            if (remark && remark.replace(/[^\x00-\xff]/g, 'xx').length > 20) {
                                dialogService.alert(modelConstant.MODEL_REMARK_TIP);
                                return;
                            }

                            modelService.voiceRemark({
                    ...
                    $scope.$on('preview', function (event, data) {
                        ...
                        if (data.type === 'model' && $scope.$parent.validatefragmentContent() && $scope.$parent.validateTime() && !$scope.$parent.isError && $scope.$parent.preOperateSilenceContent()) {
                            ...
                            if ($scope.model.modelFragments != '[]') {
                                ...
                                // 获取每一个片段的命中数
                                var filter = {};
                                if ($scope.$parent.b.filter) {
                                    filter = $scope.$parent.b.filter.length ? JSON.stringify($scope.$parent.b) : '';
                                    if ($scope.$parent.b.filter.length) {
                                        angular.forEach($scope.$parent.b.filter, function (item) {
                    ...
                    $scope.getCallList = function () {
                        if ($scope.$parent.status != -1 && $scope.$parent.status != 0) {
                        ...
                        modelService.getCallList(params).then(function (result) {
                            // 石勇 新增 获取所有的结果
                    ...
                    $scope.getMarkList = function () {
                        ...
                        modelService.getMarkList(params).then(function (result) {
                            // 石勇 新增 获取所有的结果
                    ...
                    $scope.voiceMark = function (item, mark) {
                        ...
                        // 获取上次的标记情况
                        modelService.getTelephonIdState({
                    ...
                    $scope.resultExport = function () {
                        ...
                        // 导出所有的通话列表结果
                        $('#ModelForm1').attr('action', modelService.exportPreviewModel()).submit();
