...
(function (factory) {
    ...
    app.controller('modelListCtrl', [
        ...
        // 石勇 新增
        'dimensionService',
        '$rootScope', function ($scope, $stateParams, $timeout, ngDialog, $q, $document, dialogService, winHeightService, modelService, baseService, modelConstant, CONSTANT, dimensionService, $rootScope) {
            ...
            $scope.findModels = function (params) {
                ...
                return modelService.findModels(params)
            ...
            $scope.modelDelete = function (id, modelName) {
                modelService.deleteModel({modelIds: id, modelName: modelName})
                    .then(function (result) {
                        $scope.findModels();
                    });
            };
            ...
            $scope.batchOperate = function (type) {
                var tip = type === 'offline' ? modelConstant.MODEL_BATCH_OFFLINE_TIP : modelConstant.MODEL_BATCH_DEL_TIP;
            ...
            $scope.oneOperate = function (id, modelName, type) {
                var tip = type === 'offline' ? modelConstant.MODEL_OFFLINE_TIP : modelConstant.MODEL_DEL_TIP; // 判断是下线还是删除
            ...
            $scope.offline = function (id, modelName) {
                modelService.offlineModel({modelIds: id, modelName: modelName})
                    .then(function (result) {
                        $scope.findModels();
                    });
            };
            ...
            $scope.searchGroup = function () {
                if (!baseService.validWord($scope.searchkeyword)) {
                    return;
                }
            ...
            $scope.setModelUp = function (id, modelName, flag) {
                modelService.setModelUp({modelId: id, modelName: modelName, deleteUp: flag})
                    .then(function (result) {
                        $scope.findModels();
                    });
            };
                ...
                $timeout(function () {
                    ...
                    if (modelIds.length) {
                        ...
                        if (modelIds.length > 1) {
                            $('#form').attr('action', modelService.exportMoreRule());
                            $('[name=\'modelId\']').val(modelIds);
                        }
                        else {
                            $('#form').attr('action', modelService.exportSingleRule());
                        }
                    }
                    else {
            ...
            $scope.getAllModelGroups = function (params) {
                params = $.extend(params, {
                    modelGroupName: $scope.searchkeyword
                });
                modelService.searchModelGroup(params)
                    .then(function (result) {
                        $scope.modelGroups = result.value || [];
                        if ($scope.groupId === -1) {
                            $scope.groupListName = '全部';
                            $scope.modelGroupId = 0;
                        }
                        else {
                            // 获取所有模型组
                            ...
                            $scope.modelGroupId = $scope.groupId;
                        }
                        $scope.findModels();
                    });
            ...
            $scope.$watch('$parent.$parent.resources', function (newValue, oldValue) {
                ...
                angular.forEach(newValue, function (resource) {
                    if (resource.link === modelConstant.MODEL_LINK) { // 模型
                        ...
                        // 支持模型新增
                        if ($.inArray(modelConstant.MODEL_ADD, $scope.optAciton) > -1) {
                            $scope.addModelAuth = true;
                        }

                        // 支持模型导出
                        if ($.inArray(modelConstant.MODEL_EXPORT, $scope.optAciton) > -1) {
                            $scope.exportModelAuth = true;
                        }

                        // 支持模型删除
                        if ($.inArray(modelConstant.MODEL_DELETE, $scope.optAciton) > -1) {
                        ...
                        // 支持模型置顶、取消置顶
                        if ($.inArray(modelConstant.MODEL_SETTOP, $scope.optAciton) > -1) {
                            $scope.setUpAuth = true;
                        }

                        // 支持查看模型通话列表
                        if ($.inArray(modelConstant.MODEL_CALLLIST, $scope.optAciton) > -1) {
                            $scope.callListAuth = true;
                        }

                        // 支持模型组新增
                        if ($.inArray(modelConstant.MODEL_GROUP_ADD, $scope.optAciton) > -1) {
                            $scope.groupAddAuth = true;
                        }

                        // 支持模型组编辑
                        if ($.inArray(modelConstant.MODEL_GROUP_EDIT, $scope.optAciton) > -1) {
                            $scope.groupEditAuth = true;
                        }

                        // 支持模型组删除
                        if ($.inArray(modelConstant.MODEL_GROUP_DEL, $scope.optAciton) > -1) {
            ...
            $scope.listExport = function () {
                ...
                if (modelIds.length > 0) {
                    ...
                    $('#form').attr('action', modelService.exportExcleMore()).submit();
                    ...
                    dialogService.confirm('是否导出全部列表').then(function (value) {
                        if (value) {
                            ...
                            $('#form').attr('action', modelService.exportExcleAll()).submit();
                            $('.exportExcle').remove(); // 运行结束后删除隐藏域，避免下次使用时会产生重复传值
                            $('#form').prepend('<input type="hidden" name="modelId" class="exportExcle">'); // 增加一个默认隐藏域，供导出规则使用
                        }
