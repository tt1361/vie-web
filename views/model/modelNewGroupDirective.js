...
(function (factory) {
     ...
     *
     */
    app.directive('modelNewGroup', [
        'RecursionHelper',
        '$timeout',
        'dialogService',
        'modelService',
        'CONSTANT', function (RecursionHelper, $timeout, dialogService, modelService, CONSTANT) {
            return {
                restrict: 'EA',
                replace: true,
                templateUrl: 'model/model-new-group-directive.htm',
                scope: {
                    item: '=',
                    groupId: '=',
                    addAuth: '@',
                    ...
                    return RecursionHelper.compile(element, function (scope, $el, arrt, controller, transcludeFn) {
                        ...
                        scope.deleteModelGroup = function () {
                            ...
                            dialogService.confirmTo('确定删除该模型组及其包含的所有模型？').then(function (value) {
                                if (value) {
                                    modelService.deleteModelGroup({modelGroupId: scope.item.id, modelGroupName: scope.item.text})
