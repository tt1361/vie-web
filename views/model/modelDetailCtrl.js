...
(function (factory) {
     ...
     *
     */
    app.controller('modelDetailCtrl', [
        '$scope',
        '$http',
        '$document',
        '$state',
        ...
        'dialogService',
        'modelService',
        'CONSTANT', function ($scope, $http, $document, $state, $timeout, dialogService, modelService, CONSTANT) {
            $timeout(function () {
            ...
            $scope.addModel = function () {
                ...
                // 调用是否重名接口
                modelService.checkModelName({
                    modelName: $scope.model.modelName,
                    modelGroupId: $scope.model.modelGroupId,
                    ...
                    if (response.status === 200 && response.data.success) { // 新模型
                        if (!$scope.modelType) {
                            var result = {
                                ...
                                group: $scope.model.modelGroupId,
                        ...
                        $state.go('main.model.add', {group: $scope.model.modelGroupId, name: $scope.model.modelName, remark: $scope.model.modelComment, id: -1}, {
                            reload: true
                        });
                    }
                    else {
