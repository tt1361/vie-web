...
(function (factory) {
    ...
    app.directive('mulSelModelDimension', ['$timeout', '$document', function ($timeout, $document) {
        return {
            restrict: 'EA',
            replace: true,
            scope: {
            ...
            link: function (scope, element, attrs) {
                ...
                else if (!scope.item.value[0].key) {
                    scope.arr = angular.copy(scope.item);
                    scope.item.value = [];
                    angular.forEach(scope.arr.value, function (item) {
                        scope.item.value.push({
                            key: item,
                            value: item
                        });
                    });
                ...
                scope.checkedThis = function (item) {
                    ...
                    var allchecked = true;
                    angular.forEach(scope.dimensions, function (key, item) {
                        if (!item.isSelect) {
                            allchecked = false;
                            return false;
                        }
                ...
                scope.changeDimension = function () {
                    ...
                    scope.$parent.filter.value = result.conditions;
