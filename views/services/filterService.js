...
(function (factory) {
    ...
    app.filter('celData', function () {
    ...
    }).filter('celDataPlay_task', function () {
        return function (header, map) {
            if (header === 'duration') {
                return Math.round(map[header] / 1000);
            }

            if (header === 'id' & map[header].length > 20) {
                return map[header].substring(0, 20) + '...';
            }

            return map[header];
        };
    }).filter('celDataPlayPlus', function () {
        return function (header, map) {
            if (header === 'childDuration' || header === 'childSilenceLong') {
                return Math.round(map[header] / 1000);
            }
        ...
        return function (list, length) {
            ...
            if (listNew.length > length) {
                return listNew.slice(0, length);
            }

            return listNew;
        };
    }).filter('subList_search', function () {
        return function (list, length) {
            if (!list) {
                return;
            }
    ...
    }).filter('showKeyLength_voice', function () {
        return function (list, length) {
            if (!list) {
                return false;
            }
    ...
    }).filter('showKeyLength_search', function () {
        return function (list, length) {
            if (!list) {
                return false;
            }
        ...
        return function (item, maxSize) {
            if (item.dataMaps.modelName) {
                var modelList = item.dataMaps.modelName.split(',');
                if (modelList.length <= maxSize) {
                    return item.dataMaps.modelName;
                }
                else {
                    var shortList = modelList.slice(0, 2);
