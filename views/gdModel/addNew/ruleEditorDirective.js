...
(function (factory) {
     ...
     *          previewAuth：传递预览权限
     */
    app.directive('gdRuleEditor', [
        '$document',
        'timestamp',
        'gdModelService',
        'dialogService',
        'gdModelConstant', function ($document, timestamp, gdModelService, dialogService, gdModelConstant) {
            return {
                restrict: 'EA',
                replace: true,
                templateUrl: 'gdModel/addNew/rule-editor-directive.htm',
                scope: {
                    model: '=',
                    status: '@',
                    previewAuth: '@',
                ...
                link: function ($scope, element, attrs) {
                    ...
                    $scope.exportModelFramnet = function () {
                        ...
                        $('#form_export_model').attr('action', gdModelService.exportRule()).submit();
                    };
                    ...
                    $scope.validFrament = function () {
                        if ($scope.$parent.silenceScreen.condition.length === 0) {
                            ...
                            if (!$scope.model.modelFragments.length) {
                                dialogService.alert(gdModelConstant.MODEL_EXPORT_EMPTY);
                                return false;
                            }
                        }
