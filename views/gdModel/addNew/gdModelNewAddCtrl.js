...
(function (factory) {
     ...
     *      CONSTANT:   自定义基础常量
     */
    app.controller('gdModelNewAddCtrl', [
        '$rootScope',
        '$scope',
        '$timeout',
        '$stateParams',
        ...
        // 石勇 新增
        'dimensionService',
        'commonFilterService', function ($rootScope, $scope, $timeout, $stateParams, ngDialog, $q, dialogService, gdModelService, winHeightService, baseService, silenceService, gdModelConstant, CONSTANT, dimensionService, commonFilterService) {
            // 初始化权限操作
            ...
            $scope.open = true;
            // 初始化模型状态
            $scope.status = gdModelConstant.MODEL_STATUS.OPTIMIZING;
            // 是否显示静音规则
            ...
            if ($scope.model.modelId != -1) {
                ...
                gdModelService.getModel($stateParams.id).then(function (result) {
                    ...
                    gdModelService.queryCondition({
                        ...
                        // 初始化 新增页面 需要的参数
                        gdModelService.getPreviewId().then(function (result) {
                            // 初始化 previewId;
            ...
            $scope.preview = function () {
                ...
                /*石勇 新增 如果规则组合未输入，则弹窗提示*/
                if ($scope.isTabTemp == 0 && ($scope.model.modelFragmentRelation.fragmentContent == undefined || $scope.model.modelFragmentRelation.fragmentContent == '')) {
            ...
            $scope.addModel = function () {
                ...
                gdModelService.addCondition($scope.packModelParam('save')).then(function (result) {
                    $scope.model.modelId = result.value || -1;
                    dialogService.success(gdModelConstant.MODEL_SUCCESS);
                    $timeout(function () {
                    ...
                    $.each($scope.model.modelFragments, function (index, item) {
                        if (item.fragmentId == id) {
                            if ($scope.previewFrags.length < 8) {
                                ...
                                dialogService.alert(gdModelConstant.MODEL_TAB_LENGTH);
                            }
                            return false;
                        }
            ...
            $scope.validateModel = function () {
                if (!$scope.model.modelName) {
                    dialogService.alert(gdModelConstant.MODEL_NAME_EMPTY);
                    return false;
                }

                if ($scope.model.modelName.replace(/[^\x00-\xff]/g, 'xx').length > 100) {
                    dialogService.alert(gdModelConstant.MODEL_NAME_LENGTH);
                    return false;
                }

                if (CONSTANT.textReplace.test($scope.model.modelName)) {
                    dialogService.alert(gdModelConstant.MODEL_NAME_UNVALID);
                    return false;
                }

                if (!$scope.model.modelGroupId) {
                    dialogService.alert(gdModelConstant.MODEL_GROUP_EMPTY);
                    return false;
                }
                ...
                if ($scope.preIsHavaSilent
                    ...
                    if ($scope.model.pageType === 0
                        ...
                        dialogService.alert(gdModelConstant.MODEL_FTAMENT_EMPTY);
                        return false;
                    }
                }
            ...
            $scope.validatefragmentContent = function () {
                if ($scope.model.modelFragmentRelation && $scope.model.modelFragmentRelation.fragmentContent) {
                    ...
                    if (/[^\,\:\&\|\(\)\!+a-z0-9]/igm.test($scope.model.modelFragmentRelation.fragmentContent)) {
                        dialogService.alert(gdModelConstant.MODEL_FTAMENT_VALID);
                        return false;
                    }
                }
            ...
            $scope.packModelDimension = function () {
                ...
                var screenings = angular.copy($scope.valueDim);
            ...
            $scope.calculateWidth = function () {
                // 单个对象宽度
                $timeout(function () {
                    var winWidth = angular.element('.model-content-result .data-outer-header').width();
                    var rightWidth = angular.element('.model-content-result .tabs-right').width();
                    var itemHeight = 70;
            ...
            $scope.onlineModel = function () {
                ...
                gdModelService.addCondition($scope.packModelParam('save')).then(function (modelresult) {
            ...
            $scope.modelInsideOnline = function () {
                ...
                ngDialog.open({
                    ...
                    $scope.status = gdModelConstant.MODEL_STATUS.ONLING;
                    $scope.getModelStatus();
                });
            };
            ...
            $scope.getModelStatus = function () {
                gdModelService.getModelStatus({
                    ...
                    .then(function (result) {
                        ...
                        $scope.errorReason = result.value && result.value.errorReason ? result.value.errorReason : gdModelConstant.MODEL_ONLINE_ERROR;
                        if ($scope.status != gdModelConstant.MODEL_STATUS.ONLING) { // 不是上线中状态
                            if ($scope.status === gdModelConstant.MODEL_STATUS.FAILURE) {
            ...
            $scope.offlineModel = function () {
                gdModelService.offlineModel({
                    modelIds: $scope.model.modelId
                })
                    .then(function (result) {
                        $scope.status = gdModelConstant.OPTIMIZING;
                        $scope.getModelStatus();
                    });
            };
            ...
            $scope.retBack = function () {
                if ($scope.model.modelId === -1) {
                    dialogService.confirm(gdModelConstant.MODEL_BACK_TIP).then(function () {
                        window.location.href = '#/gdModel/-1/list';
                    });
                }
                else {
            ...
            $scope.$watch('$parent.resources', function (newValue, oldValue) {
                ...
                angular.forEach(newValue, function (resource) {
                    if (resource.link === gdModelConstant.MODEL_LINK) { // 模型
                        ...
                        // 支持模型保存
                        if ($.inArray(gdModelConstant.MODEL_SAVE, $scope.optAciton) > -1) {
                            $scope.saveAuth = true;
                        }
                        ...
                        // 支持模型返回
                        if ($.inArray(gdModelConstant.MODEL_RETURN, $scope.optAciton) > -1) {
                            $scope.returnAuth = true;
                        }

                        // 支持模型修改
                        if ($.inArray(gdModelConstant.MODEL_UPDATE, $scope.optAciton) > -1) {
                            $scope.editAuth = true;
                        }

                        // 支持模型预览
                        if ($.inArray(gdModelConstant.MODEL_PREVIEW, $scope.optAciton) > -1) {
                            $scope.previewAuth = true;
                        }

                        // 支持模型搜索
                        if ($.inArray(gdModelConstant.MODEL_SEARCH, $scope.optAciton) > -1) {
                            $scope.searchAuth = true;
                        }

                        // 支持条件分配
                        if ($.inArray(gdModelConstant.MODEL_ASSIGN, $scope.optAciton) > -1) {
